...
export function CafeCustomization({
  ...
  const buildDetails = (): QuoteDetails => ({
    ...
    withVerrerie: s.serviceMode === "avec",
    boissons: s.boissons,
    hasSale: !!s.hasSale,
    ...
    >
      <LiveRecap
        rows={[
          ...
          {
            label: "Pièces salées",
            value: s.hasSale ? s.sale : null,
          },
          {
            label: "Pièces sucrées",
            value: s.hasSucre ? s.sucre : null,
          },
          {
            label: "Service",
            value:
              s.serviceMode === "avec"
                ? "Avec service — serveur sur place"
                : "Sans service — livraison + matériel jetable",
          },
        ]}
      />
